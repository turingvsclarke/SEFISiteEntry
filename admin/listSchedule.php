...
<div class="w3-main our-main" style="margin-left:250px">
  ...
  <div class="w3-row w3-padding-64">
    <a id='test' href="/n342-final/admin/judgeVerification.php">Generate new schedule</a>
        ...
        if ($_SESSION["type"] == "admin") :
            ...
            try {
                ...
                for($i=0;$i<count($judges);$i++) {
                    $schedule[$judges[$i]['FirstName'].$judges[$i]['MiddleName'].$judges[$i]['LastName']] = array();
                }

                // assign schedule information dictionaries to judge dictionaries with start time as key
                for($i=0;$i<count($rows);$i++) {
                    $schedule[$rows[$i]['FirstName'].$rows[$i]['MiddleName'].$rows[$i]['LastName']][$rows[$i]['StartTime']] = $rows[$i];
                }
                ...
                for($i=0;$i<count($judges);$i++) {
                    echo "<tr>";
                    echo "<td>".$judges[$i]['FirstName']." ".$judges[$i]['LastName']."</td>";

                    // loop through each possible start time
                    for($j=0;$j<count($times);$j++) {
                        echo "<td class='dat' id='i".$judges[$i]['JudgeID']."j".$j."' onclick='fetchSchedDetails(".$judges[$i]['JudgeID'].",".$j.")'>";
                        ...
                        if (isset($schedule[$judges[$i]['FirstName'].$judges[$i]['MiddleName'].$judges[$i]['LastName']][$times[$j]])) {
                            echo "Project Num: ".$schedule[$judges[$i]['FirstName'].$judges[$i]['MiddleName'].$judges[$i]['LastName']][$times[$j]]['ProjectNumber']."<br />";
                            echo "Booth Num: ".$schedule[$judges[$i]['FirstName'].$judges[$i]['MiddleName'].$judges[$i]['LastName']][$times[$j]]['Number']."<br />";
                        }
                        echo "</td>";
                    }

                    echo "</tr>";
                    echo "<tr class='detailRow' id='tr".$judges[$i]['JudgeID']."'></tr>";
                }
                echo "</tbody>";
                echo "</table>";
